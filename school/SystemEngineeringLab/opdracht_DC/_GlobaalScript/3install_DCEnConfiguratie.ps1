...
function MaakSharedFolderUser ($gebruiker){
    ...
    $shareNaam= "Share_{0}" -f $gebruiker.SamAccountName #("Share{0}{1}" -f $gebruiker.voornaam, $gebruiker.familienaam )
   #Write-host "$Sharenaam"
    if ( !(Get-SmbShare -name $shareNaam -ea 0) ){
...
function InstellingenDNS($NetworkId,$NetworkIdv6, $Objecten, $dnsFWName, $dnsFWAddr, $ServerName){

    #Reverse lookup zone
    try{
        Add-DnsServerPrimaryZone -ComputerName $ServerName -NetworkID $NetworkId -ReplicationScope Forest
        Add-DnsServerPrimaryZone -ComputerName $ServerName -NetworkID $NetworkIdv6 -ReplicationScope Forest
    } catch {
    ...
    function WijzigSiteName ($defaultNaam){
        ...
        function MaakGPOAan ($NewGPO) {
            ...
            Write-host "$TargetName `tbestaat nog niet als GPO en wordt aangemaakt"
                ...
                if ($TargetName -eq "CPT_disable_local_accounts") {
...
function ConfigureerDelegering {

    # Gezamenlijke variabelen:
    ...
    # Exclusion voor Agentsmith bij de GPO CPT_disable_local_accounts
...
#    New-GPLink -Guid $cptgpo.Id -Target "ou=DomainWorkstations,dc=thematrix,dc=local" -LinkEnabled Yes
    ...
    # Exclusion voor GRP_Directors bij de GPO USR_prevent-access-control-panel
...
InstellingenDNS $NetworkId $NetworkIdv6 $Objecten $dnsFWName $dnsFWAddr $DefCompName
read-host "`n`ndruk enter om verder te gaan"
...
$disableScript = "Z:\project\scripts\disable_local_users.ps1"
[string]$antwoord = Read-Host "Default locatie voor de Disable_Local_User.ps1 file: $disableScript. `nWenst u deze te wijzigen [`"Nee`": druk enter]?"
if ($antwoord -ne "") { 
    $disableScript = Read-Host "Geef het absolute pad van het Disable_Local_User.ps1-bestand in, zonder aanhalingstekens"
}
ControleerCSV($disableScript)
Write-host "`n"
Copy-Item "Z:\project\scripts\disable_local_users.ps1" -Destination "\\Agentsmith\NETLOGON"
Write-host "disable_local_users.ps1 is gecopieerd`n"
