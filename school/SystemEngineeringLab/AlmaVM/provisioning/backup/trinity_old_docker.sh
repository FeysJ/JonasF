...
server {
  ...
  server {
        ...
        location / {
                proxy_pass http://192.168.168.131:3000;
...
log "START INSTALLATION RALLLY"

#### NOG EENS CONTROLEREN WAT NODIG IS INDIEN DOCKER MAG GEBRUIKT WORDEN
...
log "make clone github repo rallly"

git clone https://github.com/lukevella/rallly-selfhosted.git

yum-config-manager --add-repo https://download.docker.com/linux/centos/docker-ce.repo

log "docker install and enable" 

yum install docker-ce docker-ce-cli containerd.io docker-compose-plugin -y

systemctl enable --now docker

log "change config file"

sed -i -r "s/SECR.*/SECRET_PASSWORD=ditmoethierminstens32karakterslangzijn/" rallly-selfhosted/config.env
sed -i -r "s/NEXT_.*/NEXT_PUBLIC_BASE_URL=rallly.thematrix.local/" rallly-selfhosted/config.env

cd rallly-selfhosted

log "initiate rallly and the database"
docker compose up -d
